...
const secrettoken =
  "mynameistejeshreddypursuingbcainmanipaluniversityaiminingtobecomefullstackdeveloper";
router.post(
  "/createuser",
    ...
    await user
      ...
      .then(async (data) => {
        ...
        if (!data) {
          return res
            .status(400)
            .send({ errors: "try logging with correct email" });
        }
        let cmpassword = await bcrypt.compare(req.body.password, data.password);
        if (!cmpassword) {
          return res
            .status(400)
            .send({ errors: "try logging with correct password" });
        }
        const used = {
          user: {
            id: data.id,
